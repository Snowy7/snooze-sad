...
export default function PersonalAnalyticsPage() {
  ...
  const analyticsGraph = useQuery(api.analytics.getAnalyticsGraph, ownerId ? { ownerId } : "skip")
  const createAnalyticsGraph = useMutation(api.analytics.createAnalyticsGraph)
  ...
  // Create analytics graph if it doesn't exist
  React.useEffect(() => {
    if (ownerId && !analyticsGraph && !isLoading) {
      setIsLoading(true)
      createAnalyticsGraph({ ownerId }).then(() => setIsLoading(false))
    }
  }, [ownerId, analyticsGraph, createAnalyticsGraph, isLoading])

  if (!analyticsGraph && isLoading) {
    return (
      <div className="relative w-full h-full">
        ...
        <div className="flex items-center justify-center h-full">
          ...
          >
            ...
            <p className="text-sm text-muted-foreground">Creating your analytics board...</p>
          </motion.div>
        </div>
      </div>
    )
  ...
  return (
    <ReactFlowProvider>
      ...
      >
        ...
        <GraphCanvas
          graphId={analyticsGraph._id as any}
          mode="personal"
          category="analytics"
        />
      </motion.div>
